...
(function (factory) {
    ...
    $.fn.dataTableExt.oApi.fnColReorder = function (oSettings, iFrom, iTo, drop, invalidateRows) {
        ...
        if (oSettings.aoColumns[iFrom].bVisible) {
            ...
            if (oSettings.nTSearch !== null) {
                nTrs = oSettings.nTSearch.getElementsByTagName('tr');
                for (i = 0, iLen = nTrs.length; i < iLen; i++) {
                    fnDomSwitch(nTrs[i], iVisibleIndex, iInsertBeforeIndex);
                }
            }
    ...
    $.extend(ColReorder.prototype, {
        ...
        "_fnConstruct": function () {
            ...
            for (i = 0; i < columnLength; i++) {
                if (i > this.s.fixed - 1 && i < columnLength - this.s.fixedRight) {
                    this._fnMouseListener(columns[i], columns[i].nTh);
                }
                
                /* Mark the original column order for later reference */
                columns[i]._ColReorder_iOrigCol = i;
            }
            ...
            if (that.s.dt.oLoadedState && that.s.dt.oLoadedState.ColWidths) {
                const stateWidths = that.s.dt.oLoadedState.ColWidths;
                if (stateWidths.length == columnLength) {
                    for (i = 0; i < columnLength; i++) {
                        columns[i].sWidthOrig = stateWidths[i];
                    }
                }
            }
        ...
        "_fnStateSave": function (oState) {
            ...
            for (i = 0, iLen = columns.length; i < iLen; i++) {
                /* save a column widths in state */
                const iOrigColumn = columns[i]._ColReorder_iOrigCol;
                oState.ColWidths[iOrigColumn] = columns[i].sWidth;
            }
        },
         ...
         *  @param   column
         *  @param   nTh TH element clicked on
         *  @returns void
         *  @private
         */
        "_fnMouseListener": function (column, nTh) {
            var that = this;
            ...
            $(nTh)
                .on('mousedown.ColReorder', function (e) {
                    e.preventDefault();
                    if (e.which == 1) {
                        that._fnMouseDown.call(that, e, nTh, column);
                    }
                })
         ...
         *  @method  _fnMouseDown
         *  @param   e Mouse event
         *  @param   nTh TH element to be dragged
         *  @param   column The column that's resized/dragged
         *  @returns void
         *  @private
         */
        "_fnMouseDown": function (e, nTh, column) {
            var
                that = this,
            ...
            function addEventsHandler() {
                ...
                $(document)
                    .on('mousemove.ColReorder touchmove.ColReorder', function (e) {
                        ...
                        that._fnMouseMove.call(that, e, column);
                    })
                    .on('mouseup.ColReorder touchend.ColReorder', function (e) {
                        e.preventDefault();
                        e.stopPropagation();
                        that._fnMouseUp.call(that, e, column);
                    });
            }
            ...
            if ($(nTh).css('cursor') === 'col-resize') {
                ...
                aoColumns[column.idx].CRbSortableCache = aoColumns[column.idx].bSortable;
                aoColumns[column.idx].bSortable = false;
                ...
                if (aoColumns[column.idx].bReorderable === false) {
                    return false;
                }
         ...
         *  @method  _fnMouseMove
         *  @param   e Mouse event
         *  @param   column Column that's being dragged or resized
         *  @returns void
         *  @private
         */
        "_fnMouseMove": function (e, column) {
            let scrollXEnabled = this.s.dt.oInit.sScrollX !== "";
            
            //Keep the current table's width (used in case sScrollX is enabled to resize the whole table, giving an Excel-like behavior)
            if ((this.table_size === undefined || this.table_size < 0) && scrollXEnabled && $('div.dataTables_scrollHead', this.s.dt.nTableWrapper) !== undefined) {
            ...
            if (this.dom.resize) {
                let nTh = this.s.mouse.resizeElem;
                let moveLength = e.pageX - this.s.mouse.startX;
                ...
                if ($('div.dataTables_scrollBody') != null && $('div.dataTables_scrollBody').length > 0) {
                    ...
                    for (currentColumnIndex = -1; currentColumnIndex < this.s.dt.aoColumns.length - 1 && currentColumnIndex !== column.idx; currentColumnIndex++) {
                        if (this.s.dt.aoColumns[currentColumnIndex + 1].bVisible)
                            visibleColumnIndex++;
                    }
                    
                    //Get the scroller's div
                    let tableScroller = $('div.dataTables_scrollBody', this.s.dt.nTableWrapper)[0];
                    
                    //Get the table
                    let scrollingTableHead = $(tableScroller)[0].childNodes[0].childNodes[0].childNodes[0];
         ...
         *  @method  _fnMouseUp
         *  @param   e Mouse event
         *  @param   column The column that was just dragged or resized.
         *  @returns void
         *  @private
         */
        "_fnMouseUp": function (e, column) {
            var that = this;
            ...
            if (this.dom.drag !== null) {
                ...
                this.s.dt.aoColumns[column.idx].bSortable = this.s.dt.aoColumns[column.idx].CRbSortableCache;
                
                //Save the new resized column's width
                this.s.dt.aoColumns[column.idx].sWidth = $(this.s.mouse.resizeElem).width() + "px";
                
                //If other columns might have changed their size, save their size too
                scrollXEnabled = this.s.dt.oInit.sScrollX !== "";
                if (!scrollXEnabled) {
                    //The column index (internal model) here might not match the visible index since some columns might have been hidden
                    for (nextVisibleColumnIndex = column.idx + 1; nextVisibleColumnIndex < this.s.dt.aoColumns.length; nextVisibleColumnIndex++) {
                        if (this.s.dt.aoColumns[nextVisibleColumnIndex].bVisible) {
                            break;
                        }
                    }
                    
                    for (previousVisibleColumnIndex = column.idx - 1; previousVisibleColumnIndex >= 0; previousVisibleColumnIndex--) {
